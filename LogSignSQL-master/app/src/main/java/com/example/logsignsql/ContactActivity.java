...
public class ContactActivity extends Activity {
    ...
    private void loadContacts() {
        ...
        while (cursor.moveToNext()) {
            @SuppressLint("Range") String name = cursor.getString(cursor.getColumnIndex(ContactDBHelper.COL_NAME));
            @SuppressLint("Range") String phone = cursor.getString(cursor.getColumnIndex(ContactDBHelper.COL_PHONE));
            mContactsList.add(name + " - " + phone);
        }
        cursor.close();
        mContactsAdapter.notifyDataSetChanged();
